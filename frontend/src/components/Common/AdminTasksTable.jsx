...
const AdminTasksTable = ({ tasks, setTasks }) => {
      ...
      <table className="w-full border-collapse border border-richblack-600">
        <thead>
          <tr className="bg-richblack-700 text-richblack-100">
            <th className="p-3 border cursor-pointer" onClick={() => handleSort("title")}>Title {getSortArrow("title")}</th>
            <th className="p-3 border">Description</th>
            <th className="p-3 border cursor-pointer" onClick={() => handleSort("dueDate")}>Due Date {getSortArrow("dueDate")}</th>
            <th className="p-3 border cursor-pointer" onClick={() => handleSort("completed")}>Status {getSortArrow("completed")}</th>
          ...
          {sortedTasks.length ? (
            sortedTasks.map((task) => (
              ...
              >
                ...
                <td className={`p-3 border font-semibold ${task.completed ? "text-caribbeangreen-400" : "text-yellow-400"}`}>
            ...
            <tr>
              <td colSpan="6" className="text-center p-4 text-richblack-400">No tasks found.</td>
            </tr>
          )}
        </tbody>
      </table>
